  ...
  return (
    <header className="sticky top-0 z-40 w-full bg-white/95 backdrop-blur-md border-b border-slate-200/80 shadow-2xs">
      <div className="w-full px-6 md:px-8 h-16 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-1.5">
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  <item.icon size={18} strokeWidth={isActive ? 2.5 : 2} className={isActive ? "text-[var(--color-primary)]" : "text-slate-400"} />
                  <span>{item.name}</span>
                </Link>
              );
            })}
        ...
        {/* Right Actions: Settings & Logout */}
        <div className="flex items-center gap-2">
          <Link
            href="/configuracoes"
            title="Configurações"
            className={`p-2 rounded-xl text-slate-500 hover:text-[var(--color-primary)] hover:bg-slate-100 transition-all ${
          ...
          <button
            ...
            title="Sair da conta"
            className="flex items-center gap-1.5 p-2 rounded-xl text-slate-500 hover:text-rose-600 hover:bg-rose-50 transition-all cursor-pointer"
          >
            <LogOut size={20} />
